...
Work on iterators (e.g., `sort(v.begin(), v.end());`)
...
#### **binary_search** - O(log N), returns bool (requires sorted)
```cpp
vector<int> v = {1, 2, 3, 4, 5};
bool found = binary_search(v.begin(), v.end(), 3);  // true
```

#### **lower_bound** - O(log N), first element >= val
```cpp
vector<int> v = {1, 2, 4, 4, 5};
auto it = lower_bound(v.begin(), v.end(), 4);      // points to first 4
```

#### **upper_bound** - O(log N), first element > val
```cpp
...
#### **unique** - O(N), removes consecutive duplicates
```cpp
...
struct Comparator {
    bool operator()(pair<int,int> a, pair<int,int> b) {
        ...
        // If tied, sort by first element ascending
        return a.first < b.first;
    }
};
...
pq.push({3, 2});
pq.push({7, 3});
cout << pq.top().first;                                // 3 (minimum)
```

### Lambda Alternative (C++11+)
```cpp
...
## 🏗️ 5. Data Structures Operations (Beyond STL)
